#OCselected-oVirt Engine Appliance kickstart file
...
repo --name="ovirt239" --baseurl=http://192.168.3.239/CI-Repos/OCselected-oVirt-4.1-testing/x86_64/ --cost=10
repo --name="ovirt159" --baseurl=http://192.168.3.159/oVirt/rpms/OCselected-oVirt41Prev/ --cost=10
...
for kv in $( ls -1v /boot/vmlinuz* |grep -v rescue |sed s/.*vmlinuz-//  ); do
  echo "title OCselected-oVirt 4.1 ($kv)" >> /boot/grub/grub.conf
...
yum install -y OCselected-engine-console
...
mkdir /.OCselected
echo $ENGINEADMINPW > /.OCselected/engineadminpw
echo $REPORTSADMINPW > /.OCselected/reportsadminpw
echo "Creating a partial answer file"
cat > /root/OCselected-engine-answers <<__EOF__
# action=setup
[environment:default]
...
echo "Deploy engine"
engine-setup --config-append=/root/OCselected-engine-answers --offline
...
cat > /etc/issue <<EOF
OCselected-oVirt Engine Appliance release 3.5.x
Kernel \r on an \m

Please login as 'engineadm' to configure the appliance
EOF

cat > /etc/OCselected-release <<EOF
OCselected-oVirt Engine Appliance release 3.5.x
EOF
...
#rm -rf /root/OCselected-engine-answers
#rm -rf /tmp/ks-*
#%end
